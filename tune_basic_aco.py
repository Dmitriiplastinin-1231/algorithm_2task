"""
Экспериментальный подбор параметров базового муравьиного алгоритма (без модификации).
...
Графики сохраняются в папку results_basic/ рядом со скриптом.
"""
...
BASE_SEED = 42
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "results_basic")
...
def run_experiment(graph, params, seed):
    """Запускает один прогон алгоритма и возвращает длину лучшего маршрута."""
    solver = AntColonyTSP(
        graph,
        ants=params["ants"],
        alpha=params["alpha"],
...
def plot_and_save(param_name, values, means, stds, output_dir, best_value):
    ...
    ax.set_title(f"Базовый АСО — подбор параметра «{param_name}»", fontsize=13)
    ax.legend()
    ax.grid(True, linestyle="--", alpha=0.5)

    fig.tight_layout()
    path = os.path.join(output_dir, f"basic_aco_{param_name}.png")
