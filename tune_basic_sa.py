"""
Экспериментальный подбор параметров базового алгоритма имитации отжига (без модификации).
...
Параметры, которые перебираются:
  ...
  - t0_multiplier    (множитель начальной температуры)
  - min_temperature  (минимальная температура, порог остановки охлаждения)

Графики сохраняются в папку results_basic_sa/ рядом со скриптом.
"""
...
BASE_SEED = 42
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "results_basic_sa")
...
def run_experiment(graph, params, seed):
    """Запускает один прогон базового отжига и возвращает длину лучшего маршрута."""
...
def plot_and_save(param_name, values, means, stds, output_dir, best_value):
    ...
    ax.plot(x, means, marker="o", color="steelblue", linewidth=2, label="Среднее")
    ax.fill_between(
        x,
    ...
    ax.set_title(f"Базовый отжиг — подбор параметра «{param_name}»", fontsize=13)
    ax.legend()
    ax.grid(True, linestyle="--", alpha=0.5)

    fig.tight_layout()
    path = os.path.join(output_dir, f"basic_sa_{param_name}.png")
...
def main():
    ...
    print("\n=== Рекомендуемые параметры (базовый отжиг) ===")
