"""
Экспериментальный подбор параметров алгоритма имитации отжига (классическая версия).
...
ACCEPTANCE_MODE = "classic"         # Режим отжига
...
def plot_results(results, param_name, xlabel, title_suffix, filename_prefix):
    ...
    fig.suptitle(
        f"Классический отжиг — влияние параметра «{title_suffix}»\n"
    ...
    # ── Длина маршрута ──
    ax = axes[0]
    ax.errorbar(x, ml, yerr=sl, fmt="o-", color="steelblue",
                ecolor="lightsteelblue", elinewidth=2, capsize=4,
    ...
    # ── Время работы ──
    ax = axes[1]
    ax.errorbar(x, mt, yerr=st, fmt="s-", color="darkorange",
                ecolor="moccasin", elinewidth=2, capsize=4,
...
def main():
    ...
    plot_results(
        ...
        filename_prefix="classic_sa_restarts",
    )
    ...
    plot_results(
        ...
        filename_prefix="classic_sa_steps",
    )
    ...
    print()
    print("=" * 60)
    print("ИТОГОВЫЕ РЕКОМЕНДАЦИИ (классический отжиг):")
